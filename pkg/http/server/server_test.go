...
func TestServer_Get(t *testing.T) {
	...
	}{{name: "drop expired samples", fields: fields{store: storeWithData(map[string][]*clientmodel.MetricFamily{"cluster-1": {family("test_1", 1000000, 1002000, 1004000), family("test_2", 1000000, 1002000, 1004000)}}), nowFn: func() time.Time {
		return time.Unix(1001+10*60, 0)
	}}, req: &http.Request{Method: "GET"}, wantFamilies: []*clientmodel.MetricFamily{family("test_1", -1, -1), family("test_2", -1, -1)}, wantCode: 200}}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &Server{maxSampleAge: 10 * time.Minute, store: tt.fields.store, validator: tt.fields.validator, nowFn: tt.fields.nowFn}
